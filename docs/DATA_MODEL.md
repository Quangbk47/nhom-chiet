...
interface TemperatureMetadata { status: TemperatureStatus; valueC: number | null; }
...
interface SimulationInput {
  ...
  temperature: TemperatureMetadata & { valueC: number | string | null };
...
Normalized input contains finite canonical values: concentrations `mol/L`, volumes `L`, KD dimensionless, and declared temperature in °C. It is created once at the boundary and frozen for a run.
...
UTF-8 CSV required headers: `condition_id,replicate_id,stage_number,cr_mol_per_l`; optional `temperature_c,operator,notes`. Validate finite nonnegative CR, existing condition/stage, unique condition+replicate+stage and metadata match. Preview all errors; save all rows atomically or none.
