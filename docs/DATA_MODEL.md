...
Phase 12 implements only `simulation_run` snapshot serialization and immutable `experiment_replicate` batch serialization behind `src/firebase/`. The adapter accepts serialized documents through a typed driver; it does not import or invoke the calculation engine. No Firestore collection is writable under the checked-in rules until Authentication, ownership boundaries, privacy/data-retention rules and Project Owner approval are recorded.
